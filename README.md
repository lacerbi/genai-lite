...
- ⚡ **Text Streaming** - Async iterable token deltas for llama.cpp and OpenRouter
...
Streaming is currently implemented for `llamacpp` and `openrouter`. The final `complete.response` event contains the same normalized response shape returned by `sendMessage()`.
