...
- Sampling parameters: `temperature` and `topP` are mutually exclusive; if neither is selected, genai-lite sends only its temperature default. Supports `topK`. Does not support `seed`, `minP`, `repeatPenalty`, or `logprobs`/`topLogprobs` (silently stripped)
