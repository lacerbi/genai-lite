...
**Image failures share this taxonomy** (since v0.10.0): `ImageFailureResponse.error` carries the adapter's `code`/`type`/`status`/`retryAfterMs` — e.g. a busy genai-electron server surfaces as `RATE_LIMIT_EXCEEDED`/`rate_limit_error`, a cancelled generation as `REQUEST_ABORTED`/`abort_error`. Unknown/unmapped adapter errors fall back to `PROVIDER_ERROR`/`server_error`. Since v0.11.0 `ImageService` also retries transient failures for retry-safe providers (OpenAI Images; never genai-electron) and supports per-call `timeoutMs`. See [Image Service - Error Handling](image-service.md#error-handling) for the genai-electron code mappings, [Cancellation](image-service.md#cancellation), [Per-Request Timeout](image-service.md#per-request-timeout), and [Automatic Retries](image-service.md#automatic-retries).
