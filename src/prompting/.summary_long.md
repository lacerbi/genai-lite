...
- **Response Parsing**:
  ...
  - `extractMarkerDelimitedContent` from `./parser` - Extract content between an arbitrary open/close marker pair (e.g. local-model reasoning traces)
...
- **extractMarkerDelimitedContent(content, openMarker, closeMarker)**: Extracts the text between an arbitrary open/close marker pair (returns `{ content, extracted }`); used for local-model reasoning-trace cleanup
...
3. **Parser Layer** (`parser.ts`): Parses role-tagged templates and extracts structured data from responses
