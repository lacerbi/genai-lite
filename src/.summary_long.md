...
inspection/scope-keyed answer accounting/termination/usage evidence, provider-agnostic
endpoint-revision provider and binding types, token profiles and certificate
functions, and typed llama.cpp chat-input preflight responses. Endpoint
revisions are captured during preparation, exposed through inspection, and
re-read as the final gate before each physical complete attempt or streaming
dispatch. An opt-in service option may reuse binding-bearing preparation
snapshots when the host asserts revision authority; exact counting and
dispatch checks remain live. Legacy adapter methods and raw-answer accounting
remain exported and source-compatible.

The root barrel also exports the tokenizer-agnostic content-profile registry:
canonical semantic-revision computation, transactional append-only
...
The root barrel additionally exports one-position constrained-label utilities:
validated llama.cpp GBNF generation, explicit absolute/conditional/ambiguous/residual
probability extraction, and the hardened OpenAI-shape logprob mapper. These APIs reject
strict-prefix label sets, preserve terminal-stream evidence, and require full-distribution
normalization before top-N truncation for calibrated absolute/residual mass.

Alongside them the barrel exports the optional suffix-walk resolvers, their injected sync/async
fetcher types, the suffix-fragment grammar generator, and the extended result contract carrying
`resolution`, `termination`, and `fetchCount`. The resolvers build each continuation request but
never dispatch, and their suffix-resolved mass is a documented approximation.
...
- **LLMServiceOptions & PresetMode**: Configuration options for customizing the LLM service with model presets, plus a unified `retry` policy and default `timeoutMs`
- **SendMessageOptions**: Per-call `sendMessage` options (`signal`, `timeoutMs`, `maxRetries`)
- **Retry utilities**: `withRetry`, `DEFAULT_RETRY_POLICY`, and `RetryPolicy`/`RetryVerdict`/`WithRetryOptions` types (from `src/shared/services/withRetry.ts`)
...
- **OpenRouter Integration**: Support for unified access to 100+ LLM providers via OpenRouter
  - `OpenRouterClientAdapter`: Chat completion adapter for OpenRouter API gateway
  - `OpenRouterClientConfig`: Configuration type for OpenRouter adapter (baseURL, httpReferer, siteTitle)
...
- **Prompting utilities**: Template rendering, content preparation, and parsing functions
  ...
  - `parseStructuredContent`, `parseRoleTags`, `extractInitialTaggedContent`, `extractMarkerDelimitedContent`: Response parsing
...
const serverClient = new LlamaCppServerClient('http://127.0.0.1:8080');
