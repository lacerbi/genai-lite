...
## Overview
The src directory is the heart of the genai-lite library, containing the main entry point and foundational type definitions. This directory establishes the public API surface and core abstractions that make the library flexible and environment-agnostic. It serves as the boundary between the library's internal implementation and its consumers, providing a clean, well-organized interface for interacting with multiple AI providers through a unified API.
...
- **LLM types module**: All request/response/configuration types from the LLM module
...
## Usage Examples
```typescript
// Import the main service and types
import { LLMService, fromEnvironment, ApiKeyProvider } from 'genai-lite';

// Use the built-in environment provider
const service = new LLMService(fromEnvironment);

// Or implement a custom API key provider
const customProvider: ApiKeyProvider = async (providerId) => {
  return await getKeyFromDatabase(providerId);
};
