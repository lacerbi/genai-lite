...
- **LLMService class**: Main service class that orchestrates all LLM operations across different providers
  - Includes new `prepareMessage()` method for model-aware template rendering
...
- **LLM types module**: All request/response/configuration types from the LLM module, including reasoning settings
  ...
  - `PrepareMessageOptions`, `ModelContext`, `PrepareMessageResult`: Types for model-aware template rendering
...
// Use model-aware template rendering
const result = await service.prepareMessage({
  template: `{{ thinking_enabled ? "Think step-by-step:" : "Analyze:" }} {{ question }}`,
