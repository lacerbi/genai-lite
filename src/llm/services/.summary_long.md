...
- Accepts empty strings as valid content while rejecting missing, null,
  non-object, and non-string message content
...
- Threads the resolved `ModelInfo` through so detected GGUF/unknown models contribute `defaultSettings`, and applies the `reasoningDefaultSettings` overlay when reasoning is effectively on (per-key precedence: request > reasoningDefaultSettings > defaultSettings)
- Merges the new flat sampling params (topK, minP, repeatPenalty, seed), logprobs/topLogprobs, and the `llamacpp` namespace (grammar, chatTemplateKwargs)
...
- Validation of template-extracted settings with type checking and warnings (covers the new sampling params and llamacpp namespace)
...
- Accepts an exact service-provided preparation snapshot so the opt-in
  endpoint-revision cache does not trigger a second model/build/template read
...
- Mock dependencies where needed (e.g., ModelResolver mocks PresetManager from shared)
- Edge cases and error conditions thoroughly tested
- Tests serve as documentation for service behavior

## Internal Dependencies
- Services have minimal dependencies on each other:
  - ModelResolver depends on PresetManager (from src/shared/services/)
...
1. **ModelResolver**: Resolves what model to use (uses PresetManager from src/shared/services/)
2. **RequestValidator**: Validates the request structure
3. **SettingsManager**: Merges and filters settings
4. **AdapterRegistry** (from src/shared/services/): Retrieves the appropriate adapter
...
// ModelResolver usage (uses PresetManager from shared)
