...
- `LlamaCppClientAdapter.ts`: Implements llama.cpp chat/prepared integration, sharing one model/build/template snapshot with model resolution, bracketing exact input-token counts with state checks by default or endpoint revisions when explicitly authoritative, and rejecting stale dispatch.
