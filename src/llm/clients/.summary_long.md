...
evidence, independently scoped raw-content/provider-output answer accounting,
and raw plus normalized termination. Provider-output records require one
physical choice and sound provider-native usage; ambiguous or impossible
...
dispatch binding all refer to one observed server state. With explicit
revision-cache authority, the service replaces only the post-count state read
with a live ending revision read and may reuse the binding-bearing snapshot;
the per-call count and dispatch revalidation remain live.
...
- **AdapterRequestOptions**: Per-request transport options passed from the service layer to adapters (`signal` for aborts, `timeoutMs`), mapped to each SDK's timeout mechanism
- **ILLMClientAdapter**: Main interface defining the contract with methods:
  - `sendMessage(request, apiKey, options?)`: Core method for sending chat requests to LLM providers; now accepts optional `AdapterRequestOptions`
  - `validateApiKey()`: Optional method for API key format validation
  - `getAdapterInfo()`: Optional method for provider metadata
- **ADAPTER_ERROR_CODES**: Constant object defining standardized error codes for consistent error handling, including `REQUEST_TIMEOUT` (retryable) and `REQUEST_ABORTED` (never retried)
...
- **LlamaCppClientConfig interface**: Configuration options
  - `baseURL`: Server URL (default: http://127.0.0.1:8080 — 127.0.0.1, not localhost, to avoid a Windows IPv6-fallback stall)
...
- **Reasoning toggle**: Maps `settings.reasoning.enabled` to `chat_template_kwargs.enable_thinking` for detected hybrid models (explicit false unless requested; requires `llama-server --jinja`). The GGUF detection overlay also applies to the generic `llamacpp` modelId and llamacpp presets
- **Two-tier trace extraction**: Reads `reasoning_content`, then falls back to a marker pair (e.g. `<think>…</think>`) from `localReasoning.markers`; strips the template's nothink prefix; rejects assistant-prefill + thinking with a clear error
- **Assistant-prefill echo normalization**: Captures the exact nonempty trailing assistant message after final provider formatting, propagates it through legacy and prepared dispatch, and removes one exact echoed copy from normalized complete/streaming content while preserving raw parts, usage, logprobs, accounting, and termination evidence. Streaming composes no-thinking and prefill detection in one fail-open prefix state machine.
- **Vendor sampling defaults**: Recognized models apply `defaultSettings`/`reasoningDefaultSettings` over llama.cpp's server defaults
- **Extra controls**: Raw GBNF `grammar` and `chatTemplateKwargs` via the `llamacpp` namespace; per-token logprobs mapped through the shared `logprobsUtils`
...
- Forwards unified reasoning settings (effort/max_tokens/enabled/exclude) and extracts `choice.reasoning`/`reasoning_details`; unknown OpenRouter models are assumed reasoning-capable
- Maps per-token logprobs via the shared `logprobsUtils` mapper
...
- **LlamaCppClientAdapter.test.ts**: Tests llama.cpp chat behavior including assistant-prefill echo normalization, raw evidence, sampled-token logprobs, composed prefix streaming, and fail-open partial errors
- **LlamaCppPrefillPrepared.test.ts**: Tests complete and streaming assistant-prefill capture/propagation through public `LLMService` prepared dispatch
