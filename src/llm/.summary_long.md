...
- **LLMService** (main class): Orchestrates LLM operations by coordinating specialized services
  ...
  - `sendMessage(request)`: Main method orchestrating the request pipeline through all services, now includes automatic thinking extraction post-processing
  - `createMessages(options)`: Unified prompt creation that combines template rendering, model context injection, and role tag parsing
...
- **Error Handling**: Centralized error handling with standardized failure responses
...
- **Internal**: 
  ...
  - `../prompting/parser` (extractInitialTaggedContent for thinking extraction, parseRoleTags for message creation)
...
const response = await llmService.sendMessage({ presetId: 'anthropic-claude-3-7-sonnet-20250219-thinking', messages });
...
- **LLMThinkingExtractionSettings**: Configuration for automatic extraction of thinking blocks from responses with customizable tag names
