...
- **LLMService** (main class): Orchestrates LLM operations by coordinating specialized services
  ...
  - `prepareMessage(options)`: Model-aware template rendering with context injection
...
- **Internal**: 
  ...
  - `../prompting/parser` (extractInitialTaggedContent for thinking extraction)
...
// Use model-aware template rendering (new functionality)
const result = await llmService.prepareMessage({
  template: `
    {{ thinking_enabled ? "Please think step-by-step:" : "Please analyze:" }}
    {{ question }}
