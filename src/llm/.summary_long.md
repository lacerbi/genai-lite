...
## Constrained-label probability evidence

`constrainedLabels.ts` owns a one-generated-position workflow for answer labels. It validates
Unicode labels, rejects strict-prefix sets, generates a llama.cpp GBNF grammar with one optional
leading ASCII space and no trailing newline, and extracts visible label mass from normalized
`TokenLogprob` evidence. Duplicate decoded strings are combined with log-sum-exp, omitted sampled
tokens are restored, unsafe object keys are map-safe, and statuses distinguish usable,
ambiguous, missing, unmatched, and invalid evidence. Absolute/residual fields require
full-distribution normalization before top-N truncation; conditional label probabilities occupy
a separate normalization space.

The same module optionally resolves an `ambiguous_prefix` position with
`resolveLabelProbsWithSuffixWalk` / `resolveLabelProbsWithSuffixWalkAsync`. Both drive one shared
transition state machine over a private trie frontier: an ambiguous input is recomputed from its
`rawTokenLogprob` snapshot under the supplied labels and current options, any other status is
trusted typed pass-through with zero fetches, and every continuation is delegated to an injected
caller-owned fetcher that receives the exact decoded `prefix`, the reachable `suffixes`, and a
`generateSuffixGrammar` grammar. The library never dispatches. Frontier states carry exact decoded
text, so bare and space-prefixed paths stay distinct and only identical queued prefix/node states
merge; processing is sequential, highest-mass-first with stable discovery-order ties, and bounded by
a global `maxFetches` budget defaulting to 8. Unusable returned evidence is discarded transactionally
and terminates the walk, while thrown or rejected fetcher errors propagate unchanged. Results restate
absolute, ambiguous, and residual mass so they still sum to one, renormalize conditional label
probabilities over attributed labels only, and report `resolution`, `termination`, and `fetchCount`
alongside `status`. Suffix products are documented decoded-text-state
approximations with no numerical error bound, because reissued text may retokenize.

`LLMService` reports explicit supported/unsupported/unknown logprobs capabilities from model-first
then provider metadata without credentials or network calls. A final validation pass runs only
after preset, request, provider, and model settings merge, so orphaned `topLogprobs` now fail while
cross-source `logprobs: true` remains valid. The mock adapter emits deterministic requested
evidence across complete, prepared, and terminal-stream paths.
