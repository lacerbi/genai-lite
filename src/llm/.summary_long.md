...
- **LLMService** (main class): Central service that coordinates all LLM operations
  ...
  - `sendMessage(request)`: Main method for sending chat messages to LLMs (now supports preset-based requests via `LLMChatRequestWithPreset`)
  - `prepareMessage(options)`: New method for model-aware template rendering with context injection
  ...
  - `resolveModelInfo()`: Private method for resolving model information from preset or provider/model IDs
