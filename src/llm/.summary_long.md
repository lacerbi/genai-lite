...
- **LLMService** (main class): Orchestrates LLM operations by coordinating specialized services
  ...
  - `sendMessage(request)`: Main method orchestrating the request pipeline through all services, now includes automatic thinking extraction post-processing with smart enforcement (checks native reasoning status) and preserves LLM responses in partialResponse field when validation errors occur
