...
through exact startup aliases. `LLMService` reports that quality in capability
preflight and uses a resolved content profile only when raw-content answer
accounting is missing, without disturbing provider-output evidence. Optional
recipe loading and artifact I/O stay outside the service and must finish before
the registry's first read.
