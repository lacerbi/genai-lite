...
- `LLMService.ts`: Orchestrates canonical credential-free preparation, opaque WeakMap-owned handles, immutable inspection, optional caller-owned endpoint-revision capture plus opt-in revision-authoritative preparation-state reuse, live dispatch revalidation, credentialed prepared dispatch/retry, single-terminal stream coordination, scope-keyed answer evidence retention, output-limit provenance, response post-processing, and existing convenience/template APIs.
