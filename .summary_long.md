...
The v0.17.1 follow-up removes read-triggered closure: runtime
registration is append-only, unavailable capability results may be re-queried
after additions, and mapping revisions identify complete point-in-time registry
snapshots while successful mappings remain immutable.
The same patch release also treats the adapter-less mock provider as an
...
The v0.17.3 llama.cpp compatibility fix captures an exact nonempty trailing
assistant prefill from the final provider-formatted messages and removes one
echoed copy from normalized complete and streaming content. Raw provider text,
parts, usage, logprobs, accounting, and termination evidence remain lossless.
The composed streaming prefix state machine handles no-thinking cleanup before
prefill cleanup, reconsiders divergent candidates, and fails open on normal or
error termination. Public prepared complete and streaming dispatch are covered.

The v0.18.0 constrained-answer release adds validated single-position label GBNF,
absolute/conditional/ambiguous/residual probability extraction, a hardened public
OpenAI-shape mapper, deterministic mock and terminal-stream evidence, and explicit
supported/unsupported/unknown logprobs capabilities. Strict-prefix labels are rejected,
absolute evidence requires full-distribution normalization before top-N truncation, and
orphaned `topLogprobs` now fails after all settings sources merge. The resolved issue and
plan are archived under `docs/archive/`, and the portable guide lives at
`genai-lite-docs/constrained-answer-labels.md`.

The v0.19.0 suffix-walk release makes `ambiguous_prefix` resolvable instead of terminal. Synchronous
and asynchronous resolvers reconstruct the position-0 frontier from the result's own raw evidence and
hand each continuation to a caller-injected fetcher carrying the exact decoded prefix, the reachable
suffixes, and a generated suffix grammar; the library still never dispatches. Traversal is sequential
and highest-mass-first under a global fetch budget defaulting to 8, exact decoded text keeps bare and
space-prefixed branches distinct, unusable returned evidence is discarded transactionally while
operational fetcher errors propagate, and final absolute/ambiguous/residual mass still sums to one.
Results report `resolution`, `termination`, and `fetchCount` beside `status`, and are documented as
decoded-text-state approximations with no numerical error bound. The resolved issue and plan are
...
### package.json
## Purpose
This package.json file configures genai-lite v0.18.0, a lightweight and portable toolkit designed to provide a unified interface for interacting with various Generative AI APIs. The package is published under the MIT license and maintained by Luigi Acerbi.
